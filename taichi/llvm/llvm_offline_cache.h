...
/* TODO:WILL_DELETE: llvm后端offline cache文件格式
  ...
  需要存储的文件 :
    ...
    每个Kernel(grad不grad视为两个不同的kernel)一组文件(临时的, 实际还要考虑free vars, template具体参数类型)
      时间戳+src-code --> {kernel_name}_time_code.txt (格式(binary打开): {时间戳}\n{src-code}
      llvm::Module --> {kernel_name}.ll (这里的kernel_name格式待定不能用以前那种对于相同的kernel每次运行有可能的不同的名字
      offloaded_task_name_list --> {kernel_name}_otnl.txt (格式(binary打开): {name1}\n{name2}\n...
...
struct LlvmOfflineCache {
  struct KernelCache {
    ...
    uint64 last_mtime{0}; // NOTE:WILL_DELETE: 用double不好, 但由于Python获取lmt返回的是浮点, 用C++获取要用系统API(不知Taichi有没有封装, 待定
    std::string src_code;
...
public:
  ...
  LlvmOfflineCache::KernelCache get_kernel_cache(const std::string &kernel_name, llvm::LLVMContext &llvm_ctx) {
    ...
    }
    {
      ...
      std::ifstream in(filename, std::ios::in | std::ios::binary);
      if (!in.is_open()) return {};
      while (true) { // NOTE:WILL_DELETE: 不允许空行, 整理到文档里(未来会搞二进制格式, 问题不大
        std::string line;
        std::getline(in, line, '\n');
        if (line.empty()) break;
...
public:
  ...
  void add_kernel_cache(const std::string &kernel_name, LlvmOfflineCache::KernelCache &&kernel_cache) {
    data_.kernels[kernel_name] = std::move(kernel_cache); // FIXME:WILL_DELETE: 是否要检测重复添加?
  }
  ...
  void dump() {
    ...
    for (const auto &[k, v] : data_.kernels) {
      ...
      }
      {
        ...
        if (v.module) v.module->print(llvm_os, nullptr);
        else if (v.owned_module) v.owned_module->print(llvm_os, nullptr);
        else TI_ASSERT(false);
      }
      {
        std::string filename = filename_prefix + "_otnl.txt";
        std::ofstream os(filename, std::ios::out | std::ios::binary);
